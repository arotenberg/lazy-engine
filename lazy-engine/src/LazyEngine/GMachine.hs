...
data DataDecl = DataDecl [CtorDecl]
    deriving (Show, Eq)
...
data Instruction
    ...
    | MakeCtor TypeName CtorName CtorFields  -- ^ [argN, ..., arg2, arg1] ++ s  -->  newNode : s
    ...
    | CtorCaseJump TypeName (Map.Map Int32 CtorCaseBranch) Label
...
data CtorCaseBranch = CtorCaseBranch CtorName Label (Map.Map LocalID FieldName)
    deriving (Show, Eq, Ord)